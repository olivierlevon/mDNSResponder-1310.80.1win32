...
mDNSexport mStatus mDNSPollRegisterSocket( SOCKET socket, int networkEvents, mDNSPollSocketCallback callback, void *context )
{
	...
	source = (PollSource *) mDNSPlatformMemAllocate( sizeof( PollSource ) );
	if ( source == NULL )
	{
	...
	{
		ret = WSAGetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollRegisterSocket: WSACreateEvent error %d %s\n", ret, win32_strerror( ret ));
	...
	{
		ret = WSAGetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollRegisterSocket: WSAEventSelect error %d %s\n", ret, win32_strerror( ret ));
		err = mStatus_UnknownErr;
		goto exit;
	}
...
exit:
	...
	if ( err != mStatus_NoError )
	{
		if ( event != WSA_INVALID_EVENT )
		{
			...
			{
				ret = WSAGetLastError();
				dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollRegisterSocket: WSACloseEvent failed %d %s\n", ret, win32_strerror( ret ) );
			}
		}
...
mDNSexport mStatus mDNSPollUnregisterSocket( SOCKET socket )
{
	...
	if ( source != NULL )
	{
		...
		if ( !ok )
		{
			...
			dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollUnregisterSocket: WSACloseEvent failed %d %s\n", ret, win32_strerror( ret ) );
		}
	...
	{
		dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollUnregisterSocket: source not found \n" );
		err = mStatus_BadParamErr;
	}
...
mDNSexport mStatus mDNSPollUnregisterEvent( HANDLE event )
{
	...
	{
		dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPollUnregisterEvent: source not found \n" );
		err = mStatus_BadParamErr;
	}
...
mDNSexport mStatus mDNSPoll( DWORD msec )
{
	...
	if ( gPoll.main.result == WAIT_FAILED )
	{
		...
		dlog( kDebugLevelVerbose, DEBUG_NAME  "mDNSPoll: WaitForMultipleObjects WAIT_FAILED %d %s\n", ret, win32_strerror( ret ) );
...
mDNSlocal mStatus PollRegisterSource( PollSource *source )
{
	...
	else
	{
		...
		if ( worker == NULL )
		{
			...
			worker = ( Worker *) mDNSPlatformMemAllocate( sizeof( Worker ) );
			if ( worker == NULL )
			{
...
mDNSlocal mStatus PollStopWorkers( void )
{
	...
	{
		DWORD ret = GetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "PollStopWorkers: SetEvent failed %d %s\n", ret, win32_strerror(ret) );
		err = mStatus_UnknownErr;
	}
	...
	{
		DWORD ret = GetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "PollStopWorkers: WaitForSingleObject failed WAIT_FAILED %d %s\n", ret, win32_strerror( ret ) );
		err = mStatus_UnknownErr;
	}
	...
	{
		DWORD ret = GetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "PollStopWorkers: ResetEvent failed %d %s\n", ret, win32_strerror( ret ) );
		err = mStatus_UnknownErr;
	}
...
mDNSlocal mStatus PollRemoveWorker( Worker *worker )
{
	...
	dlog( kDebugLevelChatty, DEBUG_NAME "PollRemoveWorker: removing worker %d\n", worker->id );

	rc = RemoveFromList( &gPoll.workers, worker );
	if ( rc == 0 )
		dlog( kDebugLevelVerbose, DEBUG_NAME "PollRemoveWorker: worker %d not found\n", worker->id );
	...
	{
		DWORD ret = GetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "PollRemoveWorker: SetEvent failed %d %s\n", ret, win32_strerror( ret ) );
		err = mStatus_UnknownErr;
	}
	...
	{
		DWORD ret = GetLastError();
		dlog( kDebugLevelVerbose, DEBUG_NAME  "PollRemoveWorker: WaitForSingleObject failed WAIT_FAILED %d %s\n", ret, win32_strerror( ret ) );
		err = mStatus_UnknownErr;
	}
...
mDNSlocal void WorkerUnregisterSource( Worker *worker, PollSource *source )
{
	...
	if ( sourceIndex == -1 )
	{
		dlog( kDebugLevelVerbose, DEBUG_NAME  "WorkerUnregisterSource: source not found in list" );
		goto exit;
	}
...
mDNSlocal void WorkerDispatch( Worker *worker )
{
	if ( worker->result == WAIT_FAILED )
	{
		...
		dlog( kDebugLevelVerbose, DEBUG_NAME  "WorkerDispatch: worker WAIT_FAILED error %d %s\n", worker->error, win32_strerror( worker->error ) );
	}
	else if ( worker->result == WAIT_TIMEOUT )
	{
		//dlog( kDebugLevelVerbose, DEBUG_NAME "WorkerDispatch: timeout\n" );
	}
	...
	else
	{
		...
		{
			dlog( kDebugLevelVerbose, DEBUG_NAME  "WorkerDispatch: waitItemIndex (%u) is >= numSources (%u)", waitItemIndex, worker->numSources );
			goto exit;
		}
		...
		if ( source->socket != INVALID_SOCKET ) // event for a socket, call socket callback
		{
			...
			if ( ret == SOCKET_ERROR )
			{
				...
				dlog( kDebugLevelVerbose, DEBUG_NAME  "WorkerDispatch: WSAEnumNetworkEvents error %d %s\n", err, win32_strerror( err ));
...
mDNSlocal unsigned WINAPI WorkerMain( LPVOID inParam )
{
	...
	while ( TRUE )
	{
		dlog( kDebugLevelChatty, DEBUG_NAME  "WorkerMain: worker thread %d will wait on main loop\n", worker->id );
		
		result = WaitForSingleObject( worker->start, INFINITE );
		if ( result == WAIT_FAILED )
		{
			DWORD ret = GetLastError();
			dlog( kDebugLevelVerbose, DEBUG_NAME  "WorkerMain: WaitForSingleObject failed WAIT_FAILED %d %s\n", ret, win32_strerror( ret ) );
			break;
		}
		...
		dlog( kDebugLevelChatty, DEBUG_NAME "WorkerMain: worker thread %d will wait on sockets\n", worker->id );
		...
		dlog( kDebugLevelChatty, DEBUG_NAME "WorkerMain: worker thread %d did wait on sockets: %d\n", worker->id, worker->result );
		...
		dlog( kDebugLevelChatty, DEBUG_NAME  "WorkerMain: worker thread %d preparing to sleep\n", worker->id );
